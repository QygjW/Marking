...
def draw_text_with_box(image_path, centers):
    # 读取图像
    ...
    # 计算窗口大小，确保不超过屏幕尺寸
    window_width = int(min(image.shape[1], screen_width) * 0.75)
    window_height = int(min(image.shape[0], screen_height) * 0.75)
    # 设置窗口大小
...
if __name__ == '__main__':
    base_path = r"D:\embody\3_js_agent\20241212self\images_640_480"
    # base_path = ""
    out_path = r"D:\embody\3_js_agent\20241212self\images_640_480_labeld"
    ...
    for idx in range(0, len(images)):
        ...
        while True:
            # 1、手动打坐标
            coordinates = manual_point_drawing(image_path) # (H,W)
            # coordinates.append((125, 764))
            print("坐标", coordinates)  # [(1025, 284), (443, 777), (191, 194), (872, 780)]
            coordinates_str = '_'.join('_'.join(map(str, coord)) for coord in coordinates)
            # print(coordinates_str)

            # 2、绘制box和text
            image_ndarray = draw_text_with_box(image_path, coordinates)

            # 3、保存
            # image_nname = os.path.basename(image_path).replace(" (2).jpg","")+f"_coord_{coordinates_str}.jpg"
            image_nname = os.path.basename(image_path).replace(".jpg", "") + f"_coord_{coordinates_str}.jpg"
            # image_nname = os.path.basename(image_path).replace(".png", "") + f"_coord_{coordinates_str}.jpg"
            print(image_nname)
            out_img = os.path.join(out_path, image_nname)
            ...
            if human_instruction == "0":
                cv2.imwrite(out_img, image_ndarray)
                print("保存完成！")
                break
